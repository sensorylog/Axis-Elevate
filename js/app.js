...
function initApp() {
  ...
  generateSessionBtn.addEventListener('click', () => {
    ...
    sessionInfo.textContent =
      `${session.name} (${p.trainingDays} days/week, ~${p.sessionLength} min)\n\n${session.description}`;
  });
  ...
  function renderProgress() {
    ...
    progressSummary.textContent =
      ...
      `Protein so far: ${protein}g / ${p.proteinTarget}g\n` +
...
// If onboarding already done, init immediately
if (store.state.onboardingComplete) {
  initApp();
} else {
  window.addEventListener('axis:onboardingComplete', initApp, { once: true });
}
